...
static const uint32_t CAL_SAVE_EXIT_MS = 2000;
...
void loop() {
  ...
  if (!btnEnter.stable) {
    ...
    if (calWaitReleaseAfterEnter) calWaitReleaseAfterEnter = false;
  }
  ...
  if (uiMode == UI_NAV) {
    ...
    if (!calWaitReleaseAfterEnter) {
      if (btnUp.shortPress) {
        if (calEditField == CAL_FIELD_WET) eWet = clampi(eWet + CAL_STEP, 0, eDry - 250);
        else eDry = clampi(eDry + CAL_STEP, eWet + 250, 4095);
        lastLine[1] = "";
      }
      ...
      if (btnEnter.stable && !calSaveHoldFired) {
        ...
        if (held >= CAL_SAVE_EXIT_MS) {
          ...
          if ((calDry - calWet) < 250) calDry = clampi(calWet + 250, 0, 4095);
    ...
    if (now.hour() >= WINDOW_START_H && now.hour() <= WINDOW_END_H &&
      ...
      if (key != lastAutoKey) {
        ...
        if (!tempBlock && !probeOpen) {
          if (moistPct < TARGET) startWatering(false, DUR_MIN);
        }
...

  {
    ...
    if (watering && uiMode != UI_EDIT_ROW4_SCALE) {
      ...
      char r4[180];
      snprintf(r4, sizeof(r4), "04 WATER %s  %02d:%02d  SCL %d%% %s",
